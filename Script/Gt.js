 ...
 * Documentation: http://phpgt.com/Docs/ClientSide/GtJavaScript.html
 *
 * DOMAssistant is developed by Robert Nyman/DOMAssistant team
 * Code/licensing: http://domassistant.googlecode.com/
 * Documentation: http://www.domassistant.com/documentation
 *
 * DOMAssistant is wrapped by the GT function and harmonised by default.
 * This means that no global variables will be exposed, and using other
 * JavaScript libraries that share variables such as the dollar sign will
 * not be broken.
 * 
...
(function() {
	...
	var GT = function() { 
		...
		// Must be an element reference.
		if(GT.DOMAssistant) {
			return GT.DOMAssistant.$(arguments[0]);
		}
		else {
			return arguments[0];
		}
	...
	GT.dom = function(selector, quickReference) {
		if(quickReference) {
			return GT.DOMAssistant.$$(selector);
		}
	...
	/**
	 * Used to provide code with pre-serialised result object, if the response
	 * is JSON.
	 */
	...
	GT.harmonize = function() {
		...
		if(GT.DOMAssistant) { 
			GT.DOMAssistant.harmonize();
		}
	};

	GT.unharmonize = function() {
		if(GT.DOMAssistant) {
			window.$ = GT.DOMAssistant.$;
			window.$$ = GT.DOMAssistant.$$;
		}
	}
	 ...
	 */
	GT.$ = function() {
		if(GT.DOMAssistant && window.$) {
			if(window.$ === GT.DOMAssistant.$) {
				GT.harmonize();
			}
			else {
				GT.unharmonize();
...
GT.DOMAssistant.initCore();
GT.DOMAssistant.Storage = function () {
...
GT.DOMAssistant.attach(DOMAssistant.Storage);
GT.DOMAssistant.AJAX = function () {
...
GT.DOMAssistant.attach(DOMAssistant.AJAX);
GT.DOMAssistant.CSS = function () {
...
GT.DOMAssistant.attach(DOMAssistant.CSS);
GT.DOMAssistant.Content = function () {
	var D$ = DOMAssistant.$$;
	return {
		init : function () {
...
GT.DOMAssistant.attach(DOMAssistant.Content);
GT.DOMAssistant.Events = function () {
	var handler,
		key = "_events",
...
GT.DOMAssistant.attach(DOMAssistant.Events);
GT.DOMAssistant.DOMLoad = function () {
	var DOMLoaded = false,
	DOMLoadTimer = null,
	functionsToCall = [],
	addedStrings = {},
